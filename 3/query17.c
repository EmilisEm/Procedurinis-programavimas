...
// Checks if corse repeats with different grades
int checkIfCourseRepeats(char courses[10][30], int grades[], int courseCount)
{
    int i, n;
    for (i = 0; i < courseCount; ++i)
    {
        for (n = 0; n < i; ++n)
        {
            if ((strcmp(courses[i], courses[n]) == 0) && (grades[i] != grades[n]))
            {
                return 1;
            }
        }
...
int main(int argc, char *argv[])
{
    ...
    if (db)
    {
        ...
        {                                                          // process all the student records in database
            ...
            if (checkIfCourseRepeats(s.courses, s.grades, s.load)) // *** third filter, various other conditions
            {
